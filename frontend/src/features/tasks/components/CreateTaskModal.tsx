...
export function CreateTaskModal({ isOpen, onClose, onSuccess }: CreateTaskModalProps) {
  ...
  const { meta, isLoading: isLoadingMeta, error: metaError } = useTaskCreateMeta('');
  ...
  const onSubmit = async (values: CreateTaskFormValues) => {
    ...
    try {
      ...
      const response = await createTask(values.projectId, payload as any);
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Create New Task" size="xl">
      {isLoadingMeta ? (
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          <Spinner size="lg" />
          <p className="text-sm text-wms-secondary">Loading project and team options...</p>
        </div>
      ) : metaError ? (
        <div className="text-center py-8">
          <p className="text-sm text-wms-danger font-medium">Failed to load creation options</p>
